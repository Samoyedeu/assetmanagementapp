<%-- Document : record_processing Created on : 04 6, 23, 5:30:16 PM Author :
ccslearner --%> <%@page contentType="text/html" pageEncoding="UTF-8"%> <%@page
import="java.util.*, assetmanagement.*" %>
<!DOCTYPE html>
<html>
  <head>
    ...
    <div class="asset_form">
      <form class="form_class">
        <jsp:useBean
          ...
          scope="session"
        />
        <% //Receive the values from register.html Integer v_assetid =
        Integer.parseInt(request.getParameter("assetid")); java.sql.Date
        v_activity_date =
        java.sql.Date.valueOf(request.getParameter("activity_date")); String
        ...
        request.getParameter("status"); B.asset_id = v_assetid; B.activity_date
        = v_activity_date; B.activity_description = v_activity_description;
        B.tent_start = v_tent_start_date; B.tent_end = v_tent_end_date;
        B.act_start = v_act_start_date; B.act_end = v_act_end_date; B.cost =
        v_cost; B.status = v_status; int status = B.record_assetactivity();
        if(status == 1){ %>
